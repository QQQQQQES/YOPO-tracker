...
class YopoNet:
    def __init__(self, config, weight):
        ...
        self.target_selection_score_margin = float(self.config.get('target_selection_score_margin', 1.5))
    ...
    def callback_depth(self, data):
        ...
        endstate, score, selected_action_id = self.process_output(
            ...
            return_all_preds=self.visualize or self.selection_mode == 'target_distance',
        )
    ...
    def select_action_by_target_distance(self, score_pred, objectness_pred, endstate_w, start_pos, stamp_sec):
        ...
        target_world = self.get_tracking_target_world(stamp_sec, require_recent=True, predict=False)
        ...
        if np.isfinite(min_score) and self.target_selection_score_margin >= 0.0:
            valid &= score <= min_score + self.target_selection_score_margin
        if not np.any(valid):
            self.last_selection_debug["fallback"] = True
